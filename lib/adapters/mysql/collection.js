...
var Collection = dejavu.AbstractClass.declare({
	...
	$statics: {
		...
		find: function(constraints, includes, excludes, options) {
			...
			var primaryTableName = this._options.tableName;
			var mapping = {};
			var columnsToFetch = this.$static._genFetchColumnsForCollection(this.$static, includes, excludes, this._options.tableName, null, null, mapping);
			var joins = this.$static._genFetchJoinsForCollection(this.$static, includes, excludes);
			...
			_.forEach(joins, function(join) {
				...
				on[this._options.tableName + '.' + join.field] = refCollection.getTableName() + '.' + refCollection.getPrimaryKey();
			...
			return this.$static._connection.query(prepared.text, prepared.values).then(function(results) {
				var combined = thisClass._combineResultSet(results, mapping);
		...
		findOne: function(constraints, includes, excludes, options) {
			...
			_.forEach(constraints, function(c, n) {
				constraints[this._options.tableName + '.' + n] = c;
			...
			return this.$static._connection.query(prepared.text, prepared.values).then(function(results) {
				results = thisClass._combineResultSet(results, mapping);
		...
		getChildrenOf: function(constraints, includes, excludes, options) {
			...
			var mapping = {};
			var columnsToFetch = this.$static._genFetchColumnsForCollection(this.$static, includes, excludes, primaryTableName, null, null, mapping);
			var joins = this.$static._genFetchJoinsForCollection(this.$static, includes, excludes);
		 ...
		 * @param options
		 */
		getLinkedTo: function(linkedCollection, constraints, includes, excludes, options) {
			constraints = constraints || {};
			includes = includes || [];
			excludes = excludes || [];
			options = options || {};
			...
			q.join(linkTableName);

			var on = {};

			if (isParent) {
				on[linkTableName + '.parent'] = primaryTableName + '.' + this.$static.getPrimaryKey();
			} else {
				on[linkTableName + '.child'] = primaryTableName + '.' + this.$static.getPrimaryKey();
			}

			q.on(on);

			q.join(linkedTableName);

			var linkedOn = {};

			if (isParent) {
				linkedOn[linkTableName + '.child'] = linkedTableName + '.' + linkedCollection.getPrimaryKey();
			} else {
				linkedOn[linkTableName + '.parent'] = linkedTableName + '.' + linkedCollection.getPrimaryKey();
			}

			_.forEach(constraints, function(value, field) {
				linkedOn[linkedTableName + '.' + field] = value;
			});
		...
		_genFetchColumnsForCollection: function(collection, includes, excludes, tableName, parentName, parentFieldName, mapping) {
			includes = includes || [];
			excludes = excludes || [];
			...
			_.forEach(fields, function(field, name) {
				...
				columnsToFetch[tableName + '.' + name] = (parentName ? parentName + '.' : '' ) + (parentFieldName || collectionName) + '.' + name;
			});
			...
			_.forEach(includes, function(field) {
				...
				if (fields[field].reference) {
					...
					columnsToFetch = _.merge(columnsToFetch, this.$static._genFetchColumnsForCollection(refCollection, [], [], null, collectionName, field, mapping));
				}
			}.$bind(this));
		...
		_combineResultSet: function(data, mapping) {
			...
			combined = _.map(combined, function(comb) {
				...
				if (needRecursion) {
					...
					construct[recursionKey] = this.$static._combineResultSet(comb, mapping);
				}
