...
var Collection = dejavu.AbstractClass.declare({
	...
	$statics: {
		...
		getDependencies: function() {
			return this.$static._dependencies;
		},
		...
		create: function() {
			...
			_.forEach(this._fields, function(fieldAttrs, name) {
				if (fieldAttrs.virtual) return;

				q.addColumn(_.merge(fieldAttrs, { name: name }));

				if (fieldAttrs.reference) {
					q.addReference({
						column: name,
						table: collections[fieldAttrs.reference.collection].getTableName(),
						field: fieldAttrs.reference.field
					});
				}
			});
		 ...
		 * @param options
		 */
		getDescendantsOf: function(parent, constraints, includes, excludes, options) {
			options = options || {};
			var opt = this.$static._options;
		...
		/**
		 * Create persistent instance of collection using given values
		 *
		 * @param values
		 */
		immediate: function(values) {
			return (new this.$static(values)).save();
		},

		/**
		 * Create persistent instances of collection using given array of values
		 *
		 * @param valuesArray
		 */
		batchImmediate: function(valuesArray) {
			return Promise.map(valuesArray, function(values) {
				return this.$static.immediate(values);
			}.$bind(this));
		},
		...
		_genFetchColumnsForCollection: function(components) {
			...
			collection._options.accessors = collection._options.accessors || {};
			...
			_.forEach(includes, function(field) {
				...
				if (collection._options.accessors[field]) {
					var link = collection._options.accessors[field].connection;

					var linkedCollectionName = collection._options.accessors[field].child;
					...
					if (link.order) {
						order.push(linkTableAlias + '.' + link.order + ' ' + 'asc');
					}
			...
			combined = _.map(combined, function(comb) {
				...
				if (needRecursion) {
					...
					_.forEach(comb, function(group, groupName) {
						...
						//group = _.uniq(group, groupName + '.' + colPk); // TODO: check if removing this will not break anything
	...
	$finals: {
		...
		toJSON: function() {
			return _.merge(this._values, {
				__collection: this.getCollectionName()
			});
		},
		...
		getAscendants: function(constraints, includes, excludes, options) {
			options = options || {};
			var opt = this.$static._options;
			...
			query.join(opt.tableName, 'child');

			var on = {};

			on['table1.' + opt.raw.mobius.left] = { lt : 'child.' + opt.raw.mobius.left };
			on['table1.' + opt.raw.mobius.right] = { gt : 'child.' + opt.raw.mobius.right };

			on['child.id'] = this._values['id'];
			...
			return this.$static._connection.query(prep.text, prep.values).then(function(results) {
				results = this.$static._processResults(results, r.mapping, false);
				results = results.sort(function(a, b) {
					return a[opt.raw.mobius.left] - b[opt.raw.mobius.left];
				});
				return results;
			}.$bind(this));
		},
		...
		onBeforeSave: function() {
			...
			_.forEach(fields, function(attrs, name) {
				if (name === this._primaryKey) return;
				if (attrs.virtual) return;

				if (attrs.multilang) {
					if (_values[name]) {
						values[name] = this._stringifyMultilang(_values[name]);
					} else {
						values[name] = null;
					}
				...
				if (attrs.reference) {
					if (!_values[name]) return;

					var collection = collections[attrs.reference.collection];
					var pk = collection.getPrimaryKey();
					var val = _values[name];

					if (!val.isDirty() && val[pk]) {
						values[name] = val[pk];
						return;
					}

					dependencies.push(val.save().then(function() {
						values[name] = val[pk];
					}));
			...
			if (needTimestamps) {
				...
				var updatedName = this.$static._options.updatedAt;

				if (!_values[createdName]) {
					values[createdName] = moment().format("YYYY-MM-DD HH:mm:ss");
					_values[createdName] = values[createdName];
				}

				_values[updatedName] = moment().format("YYYY-MM-DD HH:mm:ss");
			}
		...
		_saveData: function() {
			return this.onBeforeSave().then(function(values) {
				var q = query(this.$static._options.tableName);
				...
				_.forEach(this.$static._fields, function(field, name) {
					if (field.virtual) return;
					if (!field.optional && !field.primary && !field.service && !values[name]) {
				...
				return this.$static._connection.query(prepared.text, prepared.values).then(function(result) {
					return this.onAfterSave(result);
				}.$bind(this));
			}.$bind(this));
		},
		...
		_addChildObject: function(value, accessor, position) {
			var linkCollectionName = accessor.child;
			var linkCollection = this.$static._schema.collections[linkCollectionName];

			if (!(value instanceof linkCollection)) {
				value = new linkCollection(value);
			}

			return value.save().then(function() {
				var q = query(accessor.connection.tableName);

				var toInsert = {};

				toInsert['parent'] = this._values[this.$static.getPrimaryKey()];
				toInsert['child'] = value[linkCollection.getPrimaryKey()];

				if (accessor.connection.order) {
					toInsert[accessor.connection.order] = position || 0;
				}
				...
				return this.$static._connection.query(prepared.text, prepared.values).then(function(result) {
					this._values[accessor.connection.accessor].push(value);
				}.$bind(this));
			}.$bind(this));
		},

		_removeChildObject: function(value, accessor) {
			var linkCollectionName = accessor.child;
			var linkCollection = this.$static._schema.collections[linkCollectionName];
			...
			toRemove['parent'] = this._values[this.$static.getPrimaryKey()];
			toRemove['child'] = value[linkCollection.getPrimaryKey()];
			...
			return this.$static._connection.query(prepared.text, prepared.values).then(function(result) {
				if (this._values[accessor.connection.accessor] && this._values[accessor.connection.accessor].length > 0) {
					var index = this._values[accessor.connection.accessor].indexOf(value);

					if (index != -1) {
						this._values[accessor.connection.accessor] = this._values[accessor.connection.accessor].splice(index, 1);
					}
				}
			}.$bind(this));
		},
		...
		_parseMultilang: function(string) {
			...
			while(match = pattern.exec(string)) {
				match[3] = match[3].replace('=5D', ']');
				match[3] = match[3].replace('=3D', '=');
		...
		_stringifyMultilang: function(obj) {
			var result = '';
