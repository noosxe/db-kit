	...
	var def = {
		...
		initialize: function(values) {
			...
			_.forEach(this.$self._fields, function(attrs, name) {
				...
				if (!attrs.readOnly) {
					this.__defineSetter__(name, function (value) {
						...
						if (attrs.multilang && _.isString(value)) {
							throw new Error('Trying to assign string value "' + value + '" to multilang field [' + name + ']');
						}
				...
				if (attrs.reference) {
					...
					if (values[name] instanceof refCollection) {
			...
			_.forEach(this.$static._options.accessors, function(acc, name) {
				this['add' + acc.child.charAt(0).toUpperCase() + acc.child.slice(1)] = function(val, position) {
					return this._addChildObject(val, acc, position);
				}.$bind(this);

				this['remove' + acc.child.charAt(0).toUpperCase() + acc.child.slice(1)] = function(val) {
					return this._removeChildObject(val, acc);
				}.$bind(this);
			...
			if (!this._values[definition.primaryKey]) {
				this._dirty = true;
			}
		}
	};
