...
module.exports = (function() {
  ...
  var Model = function(modelName, attributes, options) {
    ...
    if (!pkFound) {
      this.attributes.id = { type: options.kit.types.SERIAL, primary: true, service: true };
      pkFound = 'id';
    }
    ...
    if (this.options.timestamps) {
      this.attributes.createdAt = { type: options.kit.types.TIMESTAMPTZ, service: true, default: 'NOW()' };
      this.attributes.updatedAt = { type: options.kit.types.TIMESTAMPTZ, service: true, default: 'NOW()' };
    }
  ...
  Model.prototype.sync = function() {
    /*
    var executor = new Executor(_.extend(_.clone(this.options), { serial: true }))
      , eventEmitter = new events.EventEmitter()
      , $this = this
      , r = queryGenerator.createTable(this.attributes, _.extend(_.clone(this.options), { executor: executor }));

    this.options.kit.query({
      text: r.q,
      name: 'create_table_' + md5(r.q),
      values: r.v
    }, _.extend(_.clone(this.options), { executor: executor })).on('done', function() {
  ...
  Model.prototype.drop = function() {
    ...
    this.options.kit.query({
      ...
      values: r.v
    }, _.clone(this.options)).on('done', function() {
        eventEmitter.emit('done', $this, { queryData: r });
    }).on('error', function(err) {
        eventEmitter.emit('error', err);
    });
