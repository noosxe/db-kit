(function() {
  ...
  module.exports = function(modelName, attributes, options) {
    var tableName = options.tableName || modelName;
    var dependencies = [];
    ...
    this.sync = function(callback) {
      ...
      var hasBeforeDelete = _.has(options, 'beforeDelete')
        && _.isObject(options.beforeDelete)
        && !_.isEmpty(options.beforeDelete);

      if (hasBeforeDelete) {

        var hasDeleteReferringFrom = _.has(options.beforeDelete, 'deleteReferringFrom')
          && _.isObject(options.beforeDelete.deleteReferringFrom)
          && !_.isEmpty(options.beforeDelete.deleteReferringFrom);

        if (hasDeleteReferringFrom) {
          var fName = 'before_delete_referring_'
            + options.beforeDelete.deleteReferringFrom.entity.getTableName();

          tasks.push(function(callback) {
            var q = functions.beforeDeleteReferringFunction(fName,
              options.beforeDelete.deleteReferringFrom.entity.getTableName(),
              options.beforeDelete.deleteReferringFrom.field, 'id');

            options.kit.query({
              name: 'create_function_' + md5(q),
              text: q,
              values: []
            }, function(err, rows, result) {
              callback(err, result);
          ...
          tasks.push(function(callback) {
            var q = triggers.dropTrigger('before_' + $this.getTableName() + '_delete', $this.getTableName());

            options.kit.query({
              name: 'drop_trigger_' + md5(q),
              text: q,
              values: []
            }, function(err, rows, result) {
              callback(err, result);
          ...
          tasks.push(function(callback) {
            var q = triggers.beforeDelete('before_' + $this.getTableName() + '_delete', $this.getTableName(), fName);

            options.kit.query({
              name: 'create_trigger_' + md5(q),
              text: q,
              values: []
            }, function(err, rows, result) {
              callback(err, result);
    ...
    var genWhere = function(where, i) {
      ...
      return { string: _.map(c, function(v) { return quote(this.getTableName()) + '.' + v; }, this).join(' AND '), values:values };
    };
