    ...
    var genFields = function(qFields, owner) {
      ...
      if (qFields && _.isArray(qFields) && qFields.length > 0) {
        _.each(qFields, function(field) {
          // TODO: field renaming temporarily turned off!
/*          if (_.isArray(field)) {
    ...
    this.find = function(query, callback) {
        ...
        _.each(query.join, function(v){
          if (_.contains(fieldNames, v)) {
            if (this.isReference(v)) {
              ...
              var ret = genFields.call(referred, null, v);
              columns[ret.tableName] = ret.columns;
      ...
      if (jData.length > 0) {
        ...
        q += _.map(jData, function(jTable) {
          return quote(jTable.tableName) + ' ON ' + jTable.on;
        }).join(', ');
      }
        ...
        if (!err) {
          ...
          returning = _.map(rows, function(row) {
            ...
            _.each(row, function(v, k) {
              ...
              if (entityName === $this.getModelName()) {
                obj[entityField] = v;
              } else {
                if (!_.has(obj, entityName) || !_.isObject(obj[entityName])) {
                  obj[entityName] = {};
                }
                obj[entityName][entityField] = v;
              }
            });
