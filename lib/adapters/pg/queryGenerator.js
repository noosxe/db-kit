...
(function() {
  ...
  module.exports = {
      ...
      _.forEach(attributes, function(attrProps, attrName) {
        ...
        if (_.isObject(attrType)) {
          switch (attrType.dataType) {
            case 'enum':
              attrType = dataTypes.createEnumType(attrType.typeName, attrType.values, _.extend(_.clone(options), {
                queryGenerator: this
              }));
              break;
          }
    ...
    insert: function(attributes, values, options) {
      ...
      _.forEach(attributes, function(attrProps, attrName) {
        ...
        if (attrProps.reference) {
          ...
          obj.save({
            ...
            v[stepIndex] = result[field];
          }).on('error', function(err) {
            options.executor.revert();
          });
      ...
      q += ' RETURNING ' + quote(options.primaryKey);
